...
class TimeValidator(AbstractCalendarValidator):
    ...
    def validate(
        ...
        #     return None
        print(f"IN TIME.VALIDATE: value: {value}, pattern: {pattern}, locale: {locale}, timezone: {time_zone}")
        if pattern is None:
            # return self._parse(value=value)
            print(f"IN TIME.VALIDATE 1: value: {value}, pattern: {pattern}, locale: {locale}, timezone: {time_zone}")
            dt = self._parse(value=value, time_zone=time_zone)
            # if time_zone is None:
            #     return dt
        
        elif locale is None:
            print(f"IN TIME.VALIDATE 2: locale: {locale}, timezone: {time_zone}")
            dt = self._parse(value=value, pattern=pattern)

        else:
            print(f"IN TIME.VALIDATE 3: locale: {locale}, timezone: {time_zone}")

            locale = locale_reg2dp(locale)
            dt = self._parse(value, pattern=pattern, locale=locale, time_zone=time_zone)
            if dt is None:
                return None
            
        # Set the timezone
        # if time_zone is None:
        #     # TODO: get default time zone OR just delete this seemingly redundant branch
        #     dt = dt.replace(tzinfo=time_zone)
        # else:
        #     dt = dt.astimezone(time_zone)
        print(f"BACK IN TIE VALIDATION: {dt} with timezone: {dt.tzinfo}")
        return dt
