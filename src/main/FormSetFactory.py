...
import logging
from typing import Optional
from xml.sax.xmlreader import Attributes
...
class FormSetFactory:
    ...
    def create_form_set(self, resources: 'ValidatorResources', 
                        language: Optional[str], 
                        country: Optional[str], 
                        variant: Optional[str]) -> 'FormSet':
        """
        ...
        if form_set:
            if self._log.isEnabledFor(logging.DEBUG):
                self._log.debug(f"FormSet[{form_set.display_key()}] found - merging.")
            return form_set
        ...
        if self._log.isEnabledFor(logging.DEBUG):
            self._log.debug(f"FormSet[{form_set.display_key()}] created.")

        return form_set

    def create_object(self, attributes: Attributes, resources: 'ValidatorResources') -> 'FormSet':
        """
